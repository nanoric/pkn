...
namespace eastl
{
	...
	protected:
		...
		static_assert(
            !(sizeof(SSOLayout) != sizeof(HeapLayout) && sizeof(value_type) >= 12), "error for size"
            );
		...
		struct Layout
		{
			...
			//Layout()                                                  { ResetToSSO(); SetSize(0); } // start as SSO by default
            ...
            //inline constexpr bool IsHeap() const EA_NOEXCEPT { return true; }
            //inline constexpr bool IsSSO() const EA_NOEXCEPT { return false; }
