...
#include "PUBGSDK\SDK\PUBG_CoreUObject_classes.hpp"

#include "SearchType.h"

template <bool find_all, size_t padding, int align, class TestFunc>
void thread_seek_memory(KProcess &process,
    TestFunc test_func,
...
)
{
    ...
    while (!inputs.empty() && !finished)
    {
        Input input;
        {
            std::lock_guard<mutex> l(input_mutex);
            if (inputs.empty())
                break;
            input = inputs.back();
            inputs.pop_back();
...
template <size_t reserve_size, int number_to_seek = -1, int offset = 0, int align = 8, size_t seek_limit = 0, class TestFunc>
seek_result_t seek_regions(KProcess & process, const MemoryRegions &regions, TestFunc test_func, int nthread = 0)
{
    ...
    for (int i = 0; i < nthread; i++)
    {
        ...
        ths.emplace_back([&]()
        {
            thread_seek_memory<number_to_seek == -1, reserve_size + align + offset, align>(process, test_func, input_mutex, inputs, output_mutex, results, atomic_number_to_seek);
        }
        );
    }
    for (auto &th : ths)
...
template <size_t reserve_size,
    ...
    seek_result_t seek_memory(KProcess & process, TestFunc test_func, int nthread = 0, RegionFilterFunc region_filter = DefaultRegionFilter())
{
    auto regions = process.readwritable_regions();
    ...
    for (const auto &region : regions)
    {
        ...
        if (!region_filter(region))
            continue;
        if (heap && process.seems_heap_address(region.base))
            regions_selected.push_back(region);
        if (executable && region.executable())
            regions_selected.push_back(region);
    }
    return seek_regions<reserve_size, number_to_seek, offset, align, seek_limit>(process, regions_selected, test_func, nthread);
}
